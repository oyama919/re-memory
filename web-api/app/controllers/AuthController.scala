    ...
    loginForm.bindFromRequest.fold(
      ...
      login => {
        userService.findByEmail(login.email)
          .map { maybeUser =>
            maybeUser.fold(BadRequest(ErrorMessage("Not Found", "NoAccount").toJson)) {
              user =>
                if (passwordService.checkPassword(login.password, user.password)) {
                  Ok("success").withSession("user_email" -> login.email)
                } else {
